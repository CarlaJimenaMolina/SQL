...
# 6. Devuelve un listado con el nombre, apellidos y email de los empleados cuyo jefe tiene un código de jefe igual a 7

select nombre, apellido1, email from empleado where codigo_jefe = 7 

# 7. Devuelve un listado de todos los pedidos que fueron rechazados en 2009.

select * from pedido where year(fecha_esperada)=2009 and estado ='Rechazado';

# 8.	¿Calcula cuántos clientes tiene cada una de las ciudades que empiezan por M?

select ciudad, count(ciudad) from cliente where ciudad like 'M%' group by ciudad;

# 9. Obtén un listado con el nombre de cada cliente y el nombre y apellido de su representante de ventas.

select c.nombre_cliente as Nombre, concat_ws('/',e.nombre, e.apellido1) as Representante
from cliente as c join empleado as e  
on c.codigo_empleado_rep_ventas = e.codigo_empleado;

# 10.	La facturación que ha tenido la empresa en toda la historia, indicando la base imponible, 
# el IVA y el total facturado. La base imponible se calcula sumando el coste del producto 
# por el número de unidades vendidas de la tabla detalle_pedido. El IVA es el 21 % de la base imponible,
...
select producto.codigo_producto, producto.precio_venta * detalle_pedido.cantidad as Base_Imponible, 
(producto.precio_venta * detalle_pedido.cantidad)*0.21 as IVA,  
(producto.precio_venta * detalle_pedido.cantidad)*1.21 as Total 
...
# 11.	 Devuelve el nombre de los clientes que han hecho pagos 
# y el nombre de sus representantes 
# junto con la ciudad de la oficina a la que pertenece el representante.


select cliente.nombre_cliente as cliente, empleado.nombre as ejecutivo_de_ventas, oficina.ciudad as ciudad_ejecutivo from pago 
...
# 12.	Devuelve un listado con el código de cliente de aquellos clientes que realizaron 
#  algún pago en 2008. Tenga en cuenta que deberá eliminar aquellos códigos de cliente 
# que aparezcan repetidos. Resuelva:
...
select cliente.codigo_cliente, pago.fecha_pago from cliente inner join pago 
where year(pago.fecha_pago) = 2008 group by cliente.codigo_cliente order by cliente.codigo_cliente;
select cliente.codigo_cliente, pago.fecha_pago from cliente inner join pago 
where date_format(pago.fecha_pago, "%Y") = 2008  group by cliente.codigo_cliente order by cliente.codigo_cliente;

# 13. Lista la dirección de las oficinas que tengan clientes en “Fuenlabrada”.

select oficina.linea_direccion1, cliente.region from oficina 
...
# 14.	La facturación que ha tenido la empresa en toda la historia, indicando la base imponible, 
...
# y el total la suma de los dos campos anteriores. 
# a. agrupada por código de producto.
# b. filtrada por los códigos que empiecen por OR.

select producto.codigo_producto, producto.precio_venta * detalle_pedido.cantidad as Base_Imponible, 
(producto.precio_venta * detalle_pedido.cantidad)*0.21 as IVA,  
(producto.precio_venta * detalle_pedido.cantidad)*1.21 asTotal 
from producto join detalle_pedido
on detalle_pedido.codigo_producto=producto.codigo_producto
where producto.codigo_producto like 'Or%' 
group by producto.codigo_producto;

# 15.	Devuelve un listado de los productos que nunca han aparecido en un pedido.

select nombre from producto left join detalle_pedido on producto.codigo_producto=detalle_pedido.codigo_producto
where detalle_pedido.codigo_producto is null;

# 16.	¿Cuál fue el pago medio por año?

select year(fecha_pago), avg(total) from pago group by year(fecha_pago) order by year(fecha_pago);

# 17. Obtener los 3 códigos de producto que han obtenido el mayor volumen de venta.

select producto.codigo_producto, sum(detalle_pedido .cantidad) from producto join detalle_pedido 
on detalle_pedido.codigo_producto=producto.codigo_producto
